...
export default function SingleClient() {
  ...
  const { data: client, isLoading, error } = useGetSingleClient({ id });
  const { isPending, mutateAsync: addNote } = useAddInternalNote();
  const { isPending: clearPending, mutateAsync: clearNotes } =
    useClearInternalNotes();
  ...
  return (
    <div className="p-6 min-h-screen bg-white">
      <Link
        to={"/clients"}
        className="bg-homeBg text-white w-fit px-4 hover:bg-homeBgGradient p-2 rounded-md flex items-center gap-2"
      >
      ...
      <div className="bg-gray-100 p-4 flex flex-col gap-2 rounded-lg mb-6">
        ...
        {client.internalNote?.length > 0 && (
          <button
            onClick={async () => {
              const data = { user: client?._id };
              await clearNotes(data);
            }}
      ...
      >
        <Box sx={style}>
          ...
          <textarea
            className="p-2 bg-gray-200 rounded-md text-black outline-none"
            rows={3}
            value={note}
          ...
          <button
            onClick={async () => {
              const data = { user: client?._id, note };
              await addNote(data);
              handleClose();
            }}
            disabled={isPending}
