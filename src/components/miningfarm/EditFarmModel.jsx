...
import { useUpdateMiningFarm } from "../../hooks/miningfarm/useMiningFarm";
import { farmContract, farmCountry } from "../../utils/dropdowns";
...
export default function EditFarmModel({ open, handleClose, farm }) {
  ...
  const { mutateAsync, isPending } = useUpdateMiningFarm();
      ...
      <Box sx={style}>
        ...
        >
          ...
          <input
            ...
            required
          />
          <label className="text-xs font-medium">Total Slots</label>
          <input
            type="number"
            className="p-2 bg-neutral-100 shadow-md outline-none"
            name="totalSlots"
            defaultValue={farm?.totalSlots}
            required
          />
