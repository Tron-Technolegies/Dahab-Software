...
import {
  useBulkMoveFarm,
  useBulkUpdateFarmStatus,
  useGetAllMiningFarms,
  useGetMinersInFarm,
} from "../../hooks/miningfarm/useMiningFarm";
...
function ChangeStatusModal({ selected, setSelected, farmId }) {
  ...
  const { isPending, mutateAsync } = useBulkUpdateFarmStatus();
  return (
    <React.Fragment>
      {selected.length > 0 && (
...
function ChangeFarmModal({ selected, setSelected, farm }) {
  ...
  const handleOpen = () => setOpen(true);
  const handleClose = () => setOpen(false);
  const { isLoading, data } = useGetAllMiningFarms();
